 ...
 */
std::vector<int> PSI_API merge_lists(const std::vector<int> &l1, const std::vector<int> &l2);
 ...
 */
std::vector<int> PSI_API contract_lists(const std::vector<int> &y, const std::vector<std::vector<int>> &A_to_y);
 ...
 */
std::vector<int> PSI_API block_list(const std::vector<int> &x_list, const std::vector<int> &x_to_y_map);

/* Args: SparseMap from x to y, maximum possible y value
 * Returns: SparseMap from y to x
 */
SparseMap PSI_API invert_map(const SparseMap &x_to_y, int ny);

/* Args: SparseMap from x to y, SparseMap from y to z
 * Returns: SparseMap from x to z
 */
SparseMap PSI_API chain_maps(const SparseMap &x_to_y, const SparseMap &y_to_z);

/* Args: SparseMap from x to y, list of pairs of type x
 * Returns: extended SparseMap from x to y
 */
SparseMap PSI_API extend_maps(const SparseMap &i_to_y, const std::vector<std::pair<int,int>> &ij_to_i_j);

/* Args: Matrix, list of row indices */
SharedMatrix PSI_API submatrix_rows(const Matrix &mat, const std::vector<int> &row_inds);

/* Args: Matrix, list of column indices */
SharedMatrix PSI_API submatrix_cols(const Matrix &mat, const std::vector<int> &col_inds);

/* Args: Matrix, list of row and column indices */
SharedMatrix PSI_API submatrix_rows_and_cols(const Matrix &mat, const std::vector<int> &row_inds, const std::vector<int> &col_inds);
