...
def select_scf_gradient(name, **kwargs):
    ...
    if func is None:
        raise ManagedMethodError(
            [__name__, name, type_var, mtd_type, reference, module]
        )
...
def select_mp2_gradient(name, **kwargs):
    ...
    if func is None:
        raise ManagedMethodError(
            [__name__, name, type_var, mtd_type, reference, module, all_electron]
        )
...
def scf_wavefunction_factory(name, ref_wfn, reference, **kwargs):
    ...
    superfunc, _disp_functor = build_functional_and_disp(
    ...
    if reference in ["RHF", "RKS"]:
        ...
        raise ValidationError(
            "SCF: Unknown reference (%s) when building the Wavefunction." % reference
        )
    ...
    df_needed |= core.get_global_option("SCF_TYPE") == "DIRECT" and core.get_option(
        "SCF", "DF_SCF_GUESS"
    )
    if df_needed:
    ...
    if core.get_option("SCF", "GUESS") == "SAPGAU":
        ...
        sapgau = core.BasisSet.build(
            wfn.molecule(), "SAPGAU_BASIS", core.get_global_option("SAPGAU_BASIS")
        )
...
def scf_helper(name, post_scf=True, **kwargs):
    ...
    if ref_wfn is not None:
        raise ValidationError(
            "Cannot seed an SCF calculation with a reference wavefunction ('ref_wfn' kwarg)."
        )
    ...
    if core.get_option("SCF", "PE") or "external_potentials" in kwargs:
        ...
        scf_molecule = c1_molecule
        core.print_out(
            """  PE does not make use of molecular symmetry: """
            """further calculations in C1 point group.\n"""
        )
        core.print_out(
    ...
    if dft_func is not None:
        if name.lower() != "scf":
            raise ValidationError(
                "dft_functional was supplied to SCF, but method name was not SCF ('%s')"
                % name
            )
        name = dft_func
        bannername = name
    ...
    if cast:
        ...
        if cast is True:
            guessbasis = corresponding_basis(core.get_global_option("BASIS"), "GUESS")[
                0
            ]
    ...
    if (core.get_option("SCF", "STABILITY_ANALYSIS") == "FOLLOW") and (
        core.get_option("SCF", "REFERENCE") != "UHF"
    ):
        raise ValidationError(
    ...
    if cast:
        ...
        base_wfn = core.Wavefunction.build(
        ...
        ref_wfn = scf_wavefunction_factory(
            name, base_wfn, core.get_option("SCF", "REFERENCE"), **kwargs
        )
    ...
    scf_wfn = scf_wavefunction_factory(
        name, base_wfn, core.get_option("SCF", "REFERENCE"), **kwargs
    )
    ...
    if (core.get_option("SCF", "GUESS") == "READ") and os.path.isfile(read_filename):
        ...
        if old_wfn.basisset().name() == scf_wfn.basisset().name():
            core.print_out(
                f"  Reading orbitals from file {read_filename}, no projection.\n\n"
            )
            ...
            pCa = scf_wfn.basis_projection(
                Ca_occ, old_wfn.nalphapi(), old_wfn.basisset(), scf_wfn.basisset()
            )
            pCb = scf_wfn.basis_projection(
                Cb_occ, old_wfn.nbetapi(), old_wfn.basisset(), scf_wfn.basisset()
            )
    ...
    elif (core.get_option("SCF", "GUESS") == "READ") and not os.path.isfile(
        read_filename
    ):
        core.print_out(
    ...
    if cast:
        ...
        pCa = ref_wfn.basis_projection(
            ref_wfn.Ca(), ref_wfn.nalphapi(), ref_wfn.basisset(), scf_wfn.basisset()
        )
        pCb = ref_wfn.basis_projection(
            ref_wfn.Cb(), ref_wfn.nbetapi(), ref_wfn.basisset(), scf_wfn.basisset()
        )
        scf_wfn.guess_Ca(pCa)
        scf_wfn.guess_Cb(pCb)
    ...
    if core.get_option("SCF", "PCM"):
        ...
        scf_wfn.set_PCM(
            core.PCM(pcmsolver_parsed_fname, pcm_print_level, scf_wfn.basisset())
        )
    ...
    if core.get_option("SCF", "DDX"):
        ...
        scf_wfn.ddx = solvent.ddx.DdxInterface(
            molecule=scf_molecule, options=ddx_options, basisset=scf_wfn.basisset()
        )
    ...
    if (not use_c1) or (scf_molecule.schoenflies_symbol() == "c1"):
        ...
        c1_basis = core.BasisSet.build(
            c1_molecule, "ORBITAL", core.get_global_option("BASIS"), quiet=True
        )
...
def run_dct(name, **kwargs):
    ...
    if core.get_global_option("DCT_TYPE") == "DF":
        ...
        proc_util.check_iwl_file_from_scf_type(
            core.get_global_option("SCF_TYPE"), ref_wfn
        )
        dct_wfn = core.dct(ref_wfn)
...
def run_dfocc(name, **kwargs):
    ...
    def set_cholesky_from(corl_type):
        if corl_type == "DF":
            ...
            raise ValidationError(
                f"""Invalid type '{corl_type}' for DFOCC. See Capabilities Table at {dtl}"""
            )
    ...
    if ref_wfn is None:
        ...
        if ref_wfn.molecule().schoenflies_symbol() != "c1":
            raise ValidationError(
                """  DFOCC does not make use of molecular symmetry: """
    ...
    if core.get_local_option("DFOCC", "MOLDEN_WRITE"):
        filename = (
            core.get_writer_file_prefix(dfocc_wfn.molecule().name()) + "_dfocc.molden"
        )
...
def run_dfocc_gradient(name, **kwargs):
    ...
    if (corl_type := method_algorithm_type(name).now) not in ["DF", "CD"]:
        raise ValidationError(
            f"Invalid type {corl_type} for DFOCC gradient. See Capabilities Table at {dtl}"
        )
...
def run_qchf(name, **kwargs):
    ...
    if ref_wfn is None:
        ...
        if ref_wfn.molecule().schoenflies_symbol() != "c1":
            raise ValidationError(
                """  QCHF does not make use of molecular symmetry: """
                """reference wavefunction must be C1.\n"""
            )
...
def run_occ(name, **kwargs):
    ...
    optstash = p4util.OptionsState(
        ["OCC", "SPIN_SCALE_TYPE"], ["OCC", "ORB_OPT"], ["OCC", "WFN_TYPE"]
    )
    ...
    # Shove variables into global space
    keep_custom_spin_scaling = core.has_option_changed(
        "OCC", "SS_SCALE"
    ) or core.has_option_changed("OCC", "OS_SCALE")
...
def run_scf(name, **kwargs):
    ...
    optstash_mp2 = p4util.OptionsState(
        ["DF_BASIS_MP2"], ["DFMP2", "MP2_OS_SCALE"], ["DFMP2", "MP2_SS_SCALE"]
    )
    ...
    if ssuper.is_c_hybrid():
        ...
        if (mp2_type := core.get_global_option("MP2_TYPE")) != "DF":
            raise ValidationError(
                f"Invalid MP2 type {mp2_type} for DF-DFT energy. See capabilities Table."
            )
        ...
        scf_wfn.set_variable(
            "{} DOUBLE-HYBRID CORRECTION ENERGY".format(ssuper.name()), vdh
        )
        returnvalue += vdh
        scf_wfn.set_variable("DFT TOTAL ENERGY", returnvalue)  # P::e SCF
        for pv, pvv in scf_wfn.variables().items():
            if pv.endswith("DISPERSION CORRECTION ENERGY") and pv.startswith(
                ssuper.name()
            ):
        ...
        core.print_out(
            "    DFT Reference Energy                  = %22.16lf\n"
            % (returnvalue - vdh)
        )
        core.print_out("    Scaled MP2 Correlation                = %22.16lf\n" % (vdh))
        core.print_out(
            "    @Final double-hybrid DFT total energy = %22.16lf\n\n" % (returnvalue)
        )
        core.tstop()
        ...
        if ssuper.name() == "MP2D":
            ...
            for pv in ["SCF TOTAL ENERGY", "SCF ITERATION ENERGY", "MP2 TOTAL ENERGY"]:
                scf_wfn.set_variable(
                    pv,
                    scf_wfn.variable(pv)
                    - scf_wfn.variable("DISPERSION CORRECTION ENERGY"),
                )

            scf_wfn.set_variable(
                "MP2D CORRELATION ENERGY",
                scf_wfn.variable("MP2 CORRELATION ENERGY")
                + scf_wfn.variable("DISPERSION CORRECTION ENERGY"),
            )
            scf_wfn.set_variable(
                "MP2D TOTAL ENERGY",
                scf_wfn.variable("MP2D CORRELATION ENERGY")
                + scf_wfn.variable("HF TOTAL ENERGY"),
            )
            scf_wfn.set_variable(
                "CURRENT ENERGY", scf_wfn.variable("MP2D TOTAL ENERGY")
            )
            scf_wfn.set_variable(
            ...
            scf_wfn.set_variable(
                "CURRENT REFERENCE ENERGY", scf_wfn.variable("SCF TOTAL ENERGY")
            )
...
def run_scf_hessian(name, **kwargs):
    ...
    if ref_wfn.functional().needs_xc():
        ref_wfn.set_variable(
            "DFT TOTAL HESSIAN", H
        )  # overwritten later for DH -- TODO when DH Hessians # P::e SCF
    else:
...
def run_dfmp2_gradient(name, **kwargs):
    ...
    dfmp2_wfn.set_variable(
        "CURRENT CORRELATION ENERGY", dfmp2_wfn.variable("MP2 CORRELATION ENERGY")
    )
...
def run_dfmp2d_gradient(name, **kwargs):
    ...
    dfmp2_wfn.set_variable(
        "MP2D CORRELATION ENERGY",
        dfmp2_wfn.variable("MP2 CORRELATION ENERGY")
        + dfmp2_wfn.variable("DISPERSION CORRECTION ENERGY"),
    )
    dfmp2_wfn.set_variable(
        "MP2D TOTAL ENERGY",
        dfmp2_wfn.variable("MP2D CORRELATION ENERGY")
        + dfmp2_wfn.variable("HF TOTAL ENERGY"),
    )
    dfmp2_wfn.set_variable("CURRENT ENERGY", dfmp2_wfn.variable("MP2D TOTAL ENERGY"))
    dfmp2_wfn.set_variable(
        "CURRENT CORRELATION ENERGY", dfmp2_wfn.variable("MP2D CORRELATION ENERGY")
    )
...
def run_ccenergy(name, **kwargs):
    ...
    """
    optstash = p4util.OptionsState(
        ["TRANSQT2", "WFN"], ["CCSORT", "WFN"], ["CCENERGY", "WFN"]
    )
...
def run_ccenergy_gradient(name, **kwargs):
    ...
    """
    optstash = p4util.OptionsState(
        ["GLOBALS", "DERTYPE"], ["CCLAMBDA", "WFN"], ["CCDENSITY", "WFN"]
    )
...
def run_bccd(name, **kwargs):
    ...
    while True:
        ...
        if bcc_iter_cnt >= core.get_option("CCENERGY", "BCCD_MAXITER"):
            ...
            core.print_out(
                "You can increase the number of BCCD iterations by changing BCCD_MAXITER.\n\n"
            )
            break
        bcc_iter_cnt += 1
    ...
    ref_wfn.set_variable(
        "CURRENT CORRELATION ENERGY", ref_wfn.variable("BCCD CORRELATION ENERGY")
    )
    ...
    if name == "bccd(t)":
        ...
        ref_wfn.set_variable(
            "B(T) CORRECTION ENERGY", ref_wfn.variable("(T) CORRECTION ENERGY")
        )
        ref_wfn.set_variable(
            "BCCD(T) TOTAL ENERGY", ref_wfn.variable("CCSD(T) TOTAL ENERGY")
        )
        ref_wfn.set_variable(
            "BCCD(T) CORRELATION ENERGY",
            ref_wfn.variable("BCCD(T) TOTAL ENERGY")
            - ref_wfn.variable("SCF TOTAL ENERGY"),
        )  # note != CCSD(T) CORRELATION ENERGY
        ref_wfn.set_variable(
            "CURRENT CORRELATION ENERGY", ref_wfn.variable("BCCD(T) CORRELATION ENERGY")
        )
...
def run_scf_property(name, **kwargs):
    ...
    if len(unknown_property):
        ...
        alt_method_name = p4util.text.find_approximate_string_matches(
            unknown_property[0], complete_options, 2
        )
        alternatives = ""
        ...
        raise ValidationError(
            "SCF Property: Feature '%s' is not recognized. %s"
            % (unknown_property[0], alternatives)
        )
...
def run_cc_property(name, **kwargs):
    ...
    if "properties" in kwargs:
        ...
        raise ValidationError(
            """The "properties" keyword is required with the property() function."""
        )
    ...
    if ("DIPOLE" in one) or ("QUADRUPOLE" in one):
        one = list(
            set(one + ["DIPOLE", "QUADRUPOLE", "MULLIKEN_CHARGES", "NO_OCCUPATIONS"])
        )
    ...
    if n_excited > 0 and (name not in ["eom-ccsd", "eom-cc2"]):
        raise ValidationError(
            """Excited state CC properties require EOM-CC2 or EOM-CCSD."""
        )

    if (name in ["eom-ccsd", "eom-cc2"]) and n_response > 0:
        raise ValidationError(
            """Cannot (yet) compute response properties for excited states."""
        )
    ...
    if n_response > 0:
        if (
            "ref_wfn" in kwargs and not kwargs["ref_wfn"].same_a_b_orbs()
        ) or core.get_option("SCF", "REFERENCE") != "RHF":
            raise ValidationError("Non-RHF CC response properties are not implemented.")
...
def run_dfmp2_property(name, **kwargs):
    ...
    """
    optstash = p4util.OptionsState(
        ["DF_BASIS_SCF"], ["DF_BASIS_MP2"], ["ONEPDM"], ["OPDM_RELAX"], ["SCF_TYPE"]
    )
    ...
    if not core.has_global_option_changed("SCF_TYPE"):
        core.set_global_option(
            "SCF_TYPE", "DF"
        )  # local set insufficient b/c SCF option read in DFMP2
        core.print_out("""    SCF Algorithm Type (re)set to DF.\n""")
    ...
    if ref_wfn is None:
        ref_wfn = scf_helper(
            name, scf_do_properties=False, use_c1=True, **kwargs
        )  # C1 certified
    ...
    if name == "scs-mp2":
        dfmp2_wfn.set_variable(
            "CURRENT ENERGY", dfmp2_wfn.variable("SCS-MP2 TOTAL ENERGY")
        )
        dfmp2_wfn.set_variable(
            "CURRENT CORRELATION ENERGY",
            dfmp2_wfn.variable("SCS-MP2 CORRELATION ENERGY"),
        )
    elif name == "mp2":
        dfmp2_wfn.set_variable("CURRENT ENERGY", dfmp2_wfn.variable("MP2 TOTAL ENERGY"))
        dfmp2_wfn.set_variable(
...
def run_eom_cc(name, **kwargs):
    ...
    if name == "eom-ccsd":
        ...
        if (user_ref != "RHF") and (user_ref != "UHF"):
            raise ValidationError(
                "Reference %s for EOM-CC2 is not available." % user_ref
            )
        core.set_local_option("TRANSQT2", "WFN", "EOM_CC2")
        core.set_local_option("CCSORT", "WFN", "EOM_CC2")
        core.set_local_option("CCENERGY", "WFN", "EOM_CC2")
...
def run_adcc(name, **kwargs):
    ...
    if parse_version(adcc.__version__) < parse_version(min_version):
        raise ModuleNotFoundError(
            "adcc version {} is required at least. Version {} was found.".format(
                min_version, adcc.__version__
            )
        )
    ...
    scf_accuracy = max(
        core.get_option("SCF", "E_CONVERGENCE"), core.get_option("SCF", "D_CONVERGENCE")
    )
    ...
    if isinstance(ref_wfn, core.UHF):
        if not core.has_option_changed("ADC", "KIND"):
            ...
            raise ValidationError(
                "For UHF references the only valid values for 'KIND' are "
                "'SPIN_FLIP' or 'ANY' and not '{}.".format(kind.upper())
            )
    elif kind not in ["singlet", "triplet", "any"]:
        raise ValidationError(
            "For RHF references the value '{}' for 'KIND' is not supported.".format(
                kind.upper()
            )
        )
    kwargs["kind"] = kind
    ...
    if "cvs" in name and "core_orbitals" not in kwargs:
        raise ValidationError(
            "If a CVS-ADC method is requested, the NUM_CORE_ORBITALS option "
            "needs to be set."
        )
    if "core_orbitals" in kwargs and "cvs" not in name:
        raise ValidationError(
            "The NUM_CORE_ORBITALS option needs to be set to '0' or absent "
    ...
    try:
        ...
        raise ValidationError(
            "Unknown exception occured while "
...
def run_adcc_property(name, **kwargs):
    ...
    if unknown_properties:
        alternatives = ""
        alt_method_name = p4util.text.find_approximate_string_matches(
            unknown_properties[0], valid_properties, 2
        )
        if alt_method_name:
            alternatives = " Did you mean? " + " ".join(alt_method_name)

        raise ValidationError(
            "ADC property: Feature '{}' is not recognized. {}".format(
                unknown_properties[0], alternatives
            )
        )
    ...
    if "DIPOLE" in properties:
        ...
        lines += [
            ind + ind + format_vector("Dipole moment (in a.u.)", hf.dipole_moment)
        ]
        ...
        if state.method.level > 1:
            ...
            lines += [
                ind
                + ind
                + format_vector("Dipole moment (in a.u.)", mp.dipole_moment(2))
            ]
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
    ...
    for excitation in state.excitations:
        ...
        props = {}
        if any(
            prop in properties for prop in ("TRANSITION_DIPOLE", "OSCILLATOR_STRENGTH")
        ):
            ...
            for method in methods:
                adc_wfn.set_variable(
                    ...
                    "ELECTRIC TRANSITION DIPOLE MOMENT (LEN)",
                    data_mat,
                )
                adc_wfn.set_variable(
                ...
                adc_wfn.set_variable(
                    f"{method} ROOT 0 -> ROOT {root_index} "
                    "ELECTRIC TRANSITION DIPOLE MOMENT (LEN) - A TRANSITION",
                    data_mat,
                )
        ...
        if "OSCILLATOR_STRENGTH" in properties:
            ...
            for method in methods:
                adc_wfn.set_variable(
                    f"{method} ROOT 0 (A) -> ROOT {root_index} (A) "
                    f"OSCILLATOR STRENGTH ({gauge_short})",
                    data,
                )
                adc_wfn.set_variable(
                    f"{method} ROOT 0 (IN A) -> ROOT {root_index} (IN A) "
                ...
                adc_wfn.set_variable(
                    f"{method} ROOT 0 -> ROOT {root_index} "
                    f"OSCILLATOR STRENGTH ({gauge_short}) - A TRANSITION",
                    data,
                )
        ...
        if "ROTATIONAL_STRENGTH" in properties:
            ...
            for method in methods:
                ...
                adc_wfn.set_variable(
                    f"{method} ROOT 0 -> ROOT {root_index} "
                    "ROTATORY STRENGTH (VEL) - A TRANSITION",
                    data,
                )
        ...
        if "DIPOLE" in properties:
            ...
            for method in methods:
                adc_wfn.set_variable(
                    f"{method} ROOT {root_index} DIPOLE MOMENT", data_mat
                )
                adc_wfn.set_variable(
                    f"{method} ROOT {root_index} DIPOLE MOMENT - A TRANSITION", data_mat
                )
                adc_wfn.set_variable(
                    f"{method} ROOT {root_index} (A) DIPOLE MOMENT", data_mat
                )
                adc_wfn.set_variable(
                    f"{method} ROOT {root_index} (IN A) DIPOLE MOMENT", data_mat
                )
...
def run_detci(name, **kwargs):
    ...
    if core.get_option("DETCI", "REFERENCE") not in ["RHF", "ROHF"]:
        raise ValidationError(
            "Reference %s for DETCI is not available."
    ...
    proc_util.print_ci_results(
        ciwfn,
        name.upper(),
        ciwfn.variable("HF TOTAL ENERGY"),
        ciwfn.variable("CURRENT ENERGY"),
        print_nos,
    )
    ...
    for lvl in range(4, 11):
        if ciwfn.has_variable(f"MP{lvl} CORRELATION ENERGY") and ciwfn.has_variable(
            f"MP{lvl - 1} CORRELATION ENERGY"
        ):
            ciwfn.set_variable(
                f"MP{lvl} CORRECTION ENERGY",
                ciwfn.variable(f"MP{lvl} CORRELATION ENERGY")
                - ciwfn.variable(f"MP{lvl - 1} CORRELATION ENERGY"),
            )
            core.set_variable(
                f"MP{lvl} CORRECTION ENERGY",
                ciwfn.variable(f"MP{lvl} CORRELATION ENERGY")
...
def run_dfep2(name, **kwargs):
    ...
    if core.get_global_option("REFERENCE") != "RHF":
        raise ValidationError(
            "DF-EP2 is not available for %s references.",
    ...
    if core.has_option_changed("DFEP2", "EP2_ORBITALS"):
        ...
        irrep_map = np.hstack(
            [
                np.ones_like(dfep2_wfn.epsilon_a().nph[x]) * x
                for x in range(dfep2_wfn.nirrep())
            ]
        )
        sort = np.argsort(eps)
    ...
    for h in range(dfep2_wfn.nirrep()):
        ...
        for i in range(len(ep2_data)):
            tmp = [
                ...
                ep2_data[i][0],
                ep2_data[i][1],
                dfep2_wfn.epsilon_a().get(h, inp_data[i]),
                inp_data[i],
            ]
            ret_eps.append(tmp)
...
def run_dlpnomp2(name, **kwargs):
    ...
    if ref_wfn is None:
        ...
        raise ValidationError(
            """  DLPNO-MP2 does not make use of molecular symmetry: """
            """reference wavefunction must be C1.\n"""
        )

    if core.get_global_option("REFERENCE") != "RHF":
        raise ValidationError(
            "DLPNO-MP2 is not available for %s references.",
            core.get_global_option("REFERENCE"),
        )
    ...
    if core.get_global_option("DLPNO_LOCAL_ORBITALS") == "IBO":
        minao = core.BasisSet.build(ref_wfn.molecule(), "MINAO_BASIS", "CC-PVTZ-MINAO")
    ...
    if name == "scs-dlpno-mp2":
        dlpnomp2_wfn.set_variable(
            "CURRENT ENERGY", dlpnomp2_wfn.variable("SCS-MP2 TOTAL ENERGY")
        )
        dlpnomp2_wfn.set_variable(
            "CURRENT CORRELATION ENERGY",
            dlpnomp2_wfn.variable("SCS-MP2 CORRELATION ENERGY"),
        )

    elif name == "dlpno-mp2":
        dlpnomp2_wfn.set_variable(
            "CURRENT ENERGY", dlpnomp2_wfn.variable("MP2 TOTAL ENERGY")
        )
        dlpnomp2_wfn.set_variable(
            "CURRENT CORRELATION ENERGY",
            dlpnomp2_wfn.variable("MP2 CORRELATION ENERGY"),
        )
...
def run_dlpnoccsd(name, **kwargs):
    ...
    """
    optstash = p4util.OptionsState(["DF_BASIS_CC"], ["SCF_TYPE"])
    ...
    if ref_wfn is None:
        ...
        raise ValidationError(
            """  DLPNO-CCSD does not make use of molecular symmetry: """
            """reference wavefunction must be C1.\n"""
        )

    if core.get_global_option("REFERENCE") != "RHF":
        raise ValidationError(
            "DLPNO-CCSD is not available for %s references.",
            core.get_global_option("REFERENCE"),
        )
    ...
    dlpnoccsd_wfn.set_variable(
        "CURRENT ENERGY", dlpnoccsd_wfn.variable("CCSD TOTAL ENERGY")
    )
    dlpnoccsd_wfn.set_variable(
        "CURRENT CORRELATION ENERGY", dlpnoccsd_wfn.variable("CCSD CORRELATION ENERGY")
    )
...
def run_dlpnoccsd_t(name, **kwargs):
    ...
    if ref_wfn is None:
        ...
        raise ValidationError(
            """  DLPNO-CCSD(T) does not make use of molecular symmetry: """
            """reference wavefunction must be C1.\n"""
        )

    if core.get_global_option("REFERENCE") != "RHF":
        raise ValidationError(
            "DLPNO-CCSD(T) is not available for %s references.",
            core.get_global_option("REFERENCE"),
        )
    ...
    core.set_local_option(
        "DLPNO", "T0_APPROXIMATION", True
    ) if name == "dlpno-ccsd(t0)" else core.set_local_option(
        "DLPNO", "T0_APPROXIMATION", False
    )
    ...
    dlpnoccsd_t_wfn.set_variable(
        "CURRENT ENERGY", dlpnoccsd_t_wfn.variable("CCSD(T) TOTAL ENERGY")
    )
    dlpnoccsd_t_wfn.set_variable(
        "CURRENT CORRELATION ENERGY",
        dlpnoccsd_t_wfn.variable("CCSD(T) CORRELATION ENERGY"),
    )
...
def run_mp2f12(name, **kwargs):
    ...
    if df_f12:
        ...
        kwargs["_force_df_basis_scf"] = (
            "DF_BASIS_F12",
            core.get_option("F12", "DF_BASIS_F12"),
            "RIFIT",
        )
        ...
        if "DF" in core.get_global_option("SCF_TYPE") or "CD" in core.get_global_option(
            "SCF_TYPE"
        ):
    ...
    if core.get_global_option("REFERENCE") != "RHF":
        raise ValidationError(
            f"MP2-F12 is not available for {core.get_global_option('REFERENCE')} references."
        )
    ...
    if ref_wfn.molecule().schoenflies_symbol() != "c1":
        raise ValidationError(
            """  MP2-F12 does not make use of molecular symmetry: """
            """reference wavefunction must be C1.\n"""
        )
    ...
    if core.get_option("F12", "MP2_TYPE") == "CONV":
        ...
        ref_wfn = run_dfmp2(
            "dfmp2", ref_wfn=ref_wfn, _force_df_basis_mp2=kwargs["_force_df_basis_scf"]
        )
    ...
    combined = qcdb.libmintsbasisset.BasisSet.pyconstruct_combined(
        mol.to_string(dtype="psi4"), keys, targets, roles, others
    )
    ...
    mp2f12_wfn.set_variable(
        "CURRENT ENERGY", mp2f12_wfn.variable("MP2-F12 TOTAL ENERGY")
    )
    mp2f12_wfn.set_variable(
        "CURRENT REFERENCE ENERGY", mp2f12_wfn.variable("HF-CABS TOTAL ENERGY")
    )
    mp2f12_wfn.set_variable(
        "CURRENT CORRELATION ENERGY", mp2f12_wfn.variable("MP2-F12 CORRELATION ENERGY")
    )
...
def run_dmrgscf(name, **kwargs):
    ...
    if core.get_option("DMRG", "DMRG_MOLDEN_WRITE"):
        filename = (
            core.get_writer_file_prefix(dmrg_wfn.molecule().name())
            + "_pseudocanonical.molden"
        )
...
def run_sapt(name, **kwargs):
    ...
    sapt_dimer, monomerA, monomerB = proc_util.prepare_sapt_molecule(
        sapt_dimer, sapt_basis
    )

    # Need to ensure consistent orbital freezing
    # between monomer and dimer computations
    monomerA_basis = core.BasisSet.build(
        monomerA, "BASIS", core.get_global_option("BASIS")
    )
    monomerB_basis = core.BasisSet.build(
        monomerB, "BASIS", core.get_global_option("BASIS")
    )
    ...
    if (core.get_option("SCF", "REFERENCE") != "RHF") and (name.upper() != "SAPT0"):
        raise ValidationError(
            'Only SAPT0 supports a reference different from "reference rhf".'
        )
    ...
    if do_delta_mp2:
        ...
        mp2_corl_interaction_e -= core.variable("MP2 CORRELATION ENERGY")
        core.set_variable(
            "SAPT MP2 CORRELATION ENERGY", mp2_corl_interaction_e
        )  # P::e SAPT
...
def run_fisapt(name, **kwargs):
    ...
    if ref_wfn is None:
        ...
        core.print_out(
            'Warning! FISAPT argument "ref_wfn" is only able to use molecule information.'
        )
        sapt_dimer = ref_wfn.molecule()
    ...
    if sapt_dimer.schoenflies_symbol() != "c1":
        core.print_out(
            "  FISAPT does not make use of molecular symmetry, further calculations in C1 point group.\n"
        )
    ...
    minao = core.BasisSet.build(
        ref_wfn.molecule(), "BASIS", core.get_global_option("MINAO_BASIS")
    )
    ref_wfn.set_basisset("MINAO", minao)
    ...
    fisapt_wfn.compute_energy(
        jk_obj, external_potentials=kwargs.get("external_potentials", None)
    )
...
def run_mrcc(name, **kwargs):
    ...
    lenv = {
        "PATH": ":".join(
            [
                os.path.abspath(x)
                for x in os.environ.get("PSIPATH", "").split(":")
                if x != ""
            ]
        )
        + ":"
    ...
    try:
        retcode = subprocess.Popen(
            [external_exe], bufsize=0, stdout=subprocess.PIPE, env=lenv
        )
    except OSError as e:
        sys.stderr.write(
    ...
    for subm in ["MP2", "CCSD"]:
        if ref_wfn.has_variable(f"{subm} TOTAL ENERGY") and core.get_option(
            "SCF", "REFERENCE"
        ) in ["RHF", "UHF"]:
            ref_wfn.set_variable(f"{subm} SINGLES ENERGY", 0.0)
            ref_wfn.set_variable(
                f"{subm} DOUBLES ENERGY", ref_wfn.variable(f"{subm} CORRELATION ENERGY")
            )

    if ref_wfn.has_variable("CCSD(T) CORRELATION ENERGY") and ref_wfn.has_variable(
        "CCSD CORRELATION ENERGY"
    ):
        ref_wfn.set_variable(
            "(T) CORRECTION ENERGY",
            ref_wfn.variable("CCSD(T) CORRELATION ENERGY")
            - ref_wfn.variable("CCSD CORRELATION ENERGY"),
        )

    if ref_wfn.has_variable("A-CCSD(T) CORRELATION ENERGY") and ref_wfn.has_variable(
        "CCSD CORRELATION ENERGY"
    ):
        ref_wfn.set_variable(
            "A-(T) CORRECTION ENERGY",
            ref_wfn.variable("A-CCSD(T) CORRELATION ENERGY")
            - ref_wfn.variable("CCSD CORRELATION ENERGY"),
        )
    ...
    try:
        ...
        if keep is False and "path" not in kwargs:
...
def run_fnocc(name, **kwargs):
    ...
    if (lbl := name.replace("fno-", "")) in ["mp3", "mp4(sdq)", "mp4"]:
        fnocc_wfn.set_variable(
            "CURRENT ENERGY", fnocc_wfn.variable(f"{lbl.upper()} TOTAL ENERGY")
        )
        fnocc_wfn.set_variable(
            "CURRENT CORRELATION ENERGY",
            fnocc_wfn.variable(f"{lbl.upper()} CORRELATION ENERGY"),
        )
        if lbl == "mp4":
            fnocc_wfn.set_variable(
                "MP4 CORRECTION ENERGY",
                fnocc_wfn.variable("MP4 CORRELATION ENERGY")
                - fnocc_wfn.variable("MP3 CORRELATION ENERGY"),
            )
...
def run_detcas(name, **kwargs):
    ...
    """
    optstash = p4util.OptionsState(
        ["DETCI", "WFN"], ["SCF_TYPE"], ["ONEPDM"], ["OPDM_RELAX"]
    )
    ...
    # The conventional case
        ...
        raise ValidationError(
            "Run DETCAS: MCSCF_TYPE %s not understood."
...
def run_efp(name, **kwargs):
    ...
    try:
        ...
        raise ValidationError(
            """Method 'efp' not available without EFP fragments in molecule"""
        )
